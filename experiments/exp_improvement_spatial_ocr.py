#!/usr/bin/env python3
"""
Experiment: Coordinate-Free Vocab + Spatial-Conditioned OCR
Phase 3: Location token elimination + cell-location-conditioned content generation

Features:
- Removes discrete coordinate tokens from vocabulary (~64% reduction)
- Cell bounding boxes predicted solely via hybrid regression head
- Regression-predicted cell location feeds back into content generation
  via a SpatialConditioningMLP added to token embeddings
- Cell content structured as textlines with <LineSep> separator
"""
import argparse
import sys
...
EXPERIMENT_NAME = "Improvement_SpatialOCR"


def main():
    parser = argparse.ArgumentParser(
        description="Coordinate-Free + Spatial-Conditioned OCR Experiment")
    parser.add_argument("--data_path", type=str, default=None,
                        help="Path to training data. Required unless --validate_only.")
    ...
    parser.add_argument("--num_inference_samples", type=int, default=3,
                        help="Number of random inference samples during validation-only")
    parser.add_argument("--debug", action="store_true",
                        help="Debug mode: limit dataset to 100 samples")
    parser.add_argument("--vocab", type=str, default=None,
                        help="Path to vocab file (.txt or .json)")
    ...
    if args.validate_only:
        ...
        use_simplified = "dataset_list" in args.data_path
